...
export function buildWeeklySummary(rows, { endDate, today = endDate } = {}) {
  ...
  let detail = "Trackinghaus is listening. The first weekly reading appears after a few visits.";
  ...
  const evidenceNote = total
    ? `${sourceLabel(strongestSource[0])} accounted for ${percentage(strongestSource[1], total)}% of this week’s reading. No individual visitor is identified.`
    : "No individual visitor is identified. Trackinghaus stores only aggregate counters.";
