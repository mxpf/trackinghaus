...
export function buildWeeklySummary(rows, { endDate, today = endDate } = {}) {
  ...
  if (primary) {
    ...
    const secondSentence =
      primary.search > 0
        ? `${primary.search} ${primary.search === 1 ? "read came" : "reads came"} through search.`
        : returning > 0
          ? `${returning} returning ${returning === 1 ? "read came" : "reads came"} from an earlier visit.`
          : `${direct} ${direct === 1 ? "read came" : "reads came"} from direct links.`;
    detail = `${primary.title} received ${primary.readers} ${primary.readers === 1 ? "read" : "reads"}. ${secondSentence}`;
  }
